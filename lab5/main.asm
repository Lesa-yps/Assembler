...
DSEG SEGMENT PARA PUBLIC USE16 'DATA'
    ...
    ; Вывод ошибки при неверном вводе номера меню
    err_menu_str db "The number of menu is invalid. Please, try enter again.$"
    arr_func_ptr dw 5 DUP (0)
DSEG ENDS
...
main:
    ...
    menu:
        ...
        ; проверка а точно ли это цифра из диапазона 1-5?
        cmp al, '5'                         ; AL <=> 35h
        ja err_menu_num                     ; если символ > 5, выводим ошибку и вновь выводим меню
        cmp al, '1'                         ; AL <=> 31h
        jl err_menu_num                     ; если символ < 1, выводим ошибку и вновь выводим меню
    ...
    err_menu_num:                           ; Обработка ошибки выбора меню
        call do_enter                       ; Выводится "enter"
        mov ah, 09h                         ; Вывод строки
        mov dx, offset err_menu_str
        int 21h                             ; выводится сообщение об ошибке ввода числа
        call do_enter                       ; Выводится "enter"
        jmp menu                            ; Снова идем к выводу меню

; Завешение программы
exit_proga proc near
    mov ax, 4c00h
    int 21h
exit_proga endp
CSEG ENDS
