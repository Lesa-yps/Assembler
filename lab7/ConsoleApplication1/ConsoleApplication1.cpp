...
/*int str_len(const char* str1)
{
	...
	int len = 0;	
		movl str1, %edi				; загружаем указатель на строку в регистр edi
		movl str1, %esi				; сохраняем адрес начала строки в регистр esi
		xorl %eax, %eax				; eax = 0 (символ для поиска нулевой (конец строки '\0'))
		movl $0ffffffffh, %ecx 		; устанавливаем максимальное количество повторов поиска в масимальное число (32 1-ы в двоичном формате)
		repne scasb					; цикл, считающий длину строки
		subl %esi, %edi				; получили разницу в адресах между нулевым символом и началом строки
		decl %edi					; исключили нулевой символ из подсчёта
		movl %edi, len				; сохраняем длину строки в переменную len
	}
	return len;
}*/
int str_len(const char str1[N]) {
    int len = 0;
    cout << str1 << " " << len << endl;
