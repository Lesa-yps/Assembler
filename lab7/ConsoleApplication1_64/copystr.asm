...
.DATA
temp_buffer BYTE 100 DUP (?) ; Выделение участка памяти (буфера) размером 100 байт
...
str_copy PROC
    ...
    mov edi, OFFSET temp_buffer             ; Указатель на временный буфер (куда копируем)
    ...
    inc ecx                                 ; увеличиваем количество копируемых байт на 1, чтоб ещё символ конца строки захватило
    mov esi, OFFSET temp_buffer             ; Указатель на временный буфер (откуда копируем)
